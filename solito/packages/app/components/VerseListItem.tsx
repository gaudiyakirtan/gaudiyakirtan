...
const VerseListItem = ({
  ...
  return (
    <View
      key={index}
      className="w-full max-w-3xl m-3 mx-auto space-y-2.5 rounded-lg bg-background"
    >
      {/* original script language */}
      <View>
        {original.map((line, index) => (
          <Text
            className="text-sm text-center text-neutral"
            key={index}
          >
            {line}
          </Text>
      ...
      <View>
        {transletaration.map((line, index) => (
          <Text
            className="text-sm text-center text-highlight font-nor"
            key={index}
          >
            {line}
          </Text>
