...
const _SongListItem = ({ title, author, uid, audio, tags }: ISongListItem) => {
  return (
    <View className="bg-background-offset flex h-14 w-full flex-row items-center rounded-xl px-2.5">
      <View className="flex-1 min-w-0">
        <View className="flex flex-row items-center w-full">
          <Text className="mr-1.5 text-sm flex-inital text-primary" numberOfLines={1} ellipsizeMode="tail" style={{flexShrink: 1 }}> {/* instead of truncate */}
            {title}
          </Text>
          <View className="bg-neutral/25 flex-shrink-0 rounded-xl px-2.5 py-0.5">
            <Text className="text-neutral text-[10px] font-medium uppercase m-auto">
              {uid}
            </Text>
          </View>
        </View>
        <View className="flex flex-row items-center">
          <Text className="text-sm text-neutral mr-1.5" numberOfLines={1} ellipsizeMode="tail" style={{flexShrink: 1 }}>
            {author}
          </Text>
      ...
      <View className="flex-row-reverse flex-shrink-0 hidden ml-2 overflow-hidden sm:flex">
        {tags.reverse().map((tag, index) => (
          ...
          >
            <Text className="text-xs text-neutral">{tag}</Text>
          </View>
        ))}
      </View>
    </View>
