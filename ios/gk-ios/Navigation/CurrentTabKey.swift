...
/// `AppNavigation`.
///
/// It exists for one job: song-detail must suppress its tab's mini-player inset
...
/// suppressing turns the gate into a comparison against each tab, which needs no clearing and
/// therefore has no ordering to race; the same record hides that tab's tab bar in compact width.
/// A pushed view has no other way to learn its tab.
